    ...
    async execute() {
        ...
        for (let zk of zookeepers) {
            ...
            let parameters = [
                ...
                images.fabric[params.version].zookeeper
            ];
            await client.checkImage(images.fabric[params.version].zookeeper);
        ...
        for (let kafka of params.kafkas) {
            ...
            let parameters = [
                ...
                '-p', '9092:9092',
                '-p', '9093:9093',
                images.fabric[params.version].kafka
            ];
            parameters = hostnames.concat(parameters);
            await client.checkImage(images.fabric[params.version].kafka);
            await client.createContainer(parameters);
            brokerId = brokerId + 1;
            brokerList.push({
                host: kafka.host,
